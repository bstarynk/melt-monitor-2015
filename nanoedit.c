...
static void *
nanoeval_displaynode_mom (struct nanoeval_mom_st *nev,
...
static void *
nanoeval_getnode_mom (struct nanoeval_mom_st *nev, struct mom_item_st *envitm,
                      const struct mom_boxnode_st *nod, int depth)
{
  ...
  const void *secondv = nanoeval_mom (nev, envitm, secondargv, depth + 1);
  MOM_DEBUGPRINTF (run, "nanoeval_getnode first son value %s depth %d",
  ...
  if ((!resv || resv == MOM_EMPTY_SLOT) && arity == 3)
    {
      ...
      const void *elsev = nanoeval_mom (nev, envitm, elseargv, depth + 1);
...
static void *
nanoeval_condnode_mom (struct nanoeval_mom_st *nev,
                       struct mom_item_st *envitm,
                       const struct mom_boxnode_st *nod, int depth)
{
  void *resv = NULL;
  ...
  for (int ix = 0; ix < (int) arity; ix++)
    {
      ...
      if (!condnod || condnod->nod_connitm != MOM_PREDEFITM (when))
        {
          resv = nanoeval_mom (nev, envitm, curcondv, depth + 1);
          break;
        }
      ...
      const void *curcondtest =
        nanoeval_mom (nev, envitm, curcondexpv, depth + 1);
...
static void *
nanoeval_ornode_mom (struct nanoeval_mom_st *nev, struct mom_item_st *envitm,
                     const struct mom_boxnode_st *nod, int depth)
{
  void *resv = NULL;
...
static void *
nanoeval_andnode_mom (struct nanoeval_mom_st *nev, struct mom_item_st *envitm,
                      const struct mom_boxnode_st *nod, int depth)
{
  void *resv = NULL;
  MOM_DEBUGPRINTF (run, "nanoeval_ornode start envitm=%s nod=%s depth#%d",
...
static void *
nanoeval_whilenode_mom (struct nanoeval_mom_st *nev,
                        struct mom_item_st *envitm,
                        const struct mom_boxnode_st *nod, int depth)
{
  void *resv = NULL;
  ...
  do
    {
      ...
      testv = nanoeval_mom (nev, envitm, testexpv, depth + 1);
      ...
      for (unsigned ix = 1; ix < (int) arity; ix++)
        {
          ...
          resv = nanoeval_mom (nev, envitm, expv, depth + 1);
        }
    }
  while (testv != NULL);
...
static void *
nanoeval_itemnode_mom (struct nanoeval_mom_st *nev,
                       struct mom_item_st *envitm,
                       const struct mom_boxnode_st *nod, int depth)
{
  void *resv = NULL;
  ...
  if (argitm != NULL)
  ...
  else
    {
      const struct mom_hashedvalue_st *val =
        nanoeval_mom (nev, envitm, argitv, depth + 1);
