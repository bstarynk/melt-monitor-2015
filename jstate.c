...
mom_dump_state (const char *dirname)
{
  ...
  // force a full GC
  {
    ...
    double ticpugc = mom_process_cpu_time () - dumper.mo_du_startelapsedtime;
    if (tirealgc > 0.3 || ticpugc > 0.2)
      MOM_INFORMPRINTF
        ("predump Garbage Collection used %.3f real %.3f cpu sec (heap size %zd Mb)",
         tirealgc, ticpugc, GC_get_heap_size () >> 20);
  }
  ...
  for (unsigned eix = 0; eix < elsiz; eix++)
    {
      ...
      if (MOM_UNLIKELY (eix % 1024 == 0)
          && mom_elapsed_real_time () > lastbelltime + 3.5)
        {
          MOM_INFORMPRINTF
            ("dumped %u object contents in %.3f real %.3f cpu sec...", eix,
             mom_elapsed_real_time () - dumper.mo_du_startelapsedtime,
