...
mo_make_string_len (const char *buf, int sz)
{
  ...
  vstr->mo_va_kind = mo_KSTRING;
  vstr->mo_va_index = 0;
  vstr->mo_va_hash = hs;
  vstr->mo_sva_size = sz;
  memcpy (vstr->mo_cstr, buf, sz);
...
mo_make_tuple_closeq (mo_sequencevalue_ty * seq)
{
  ...
  MOM_ASSERTPRINTF (seq->mo_va_kind == MOM_UNFILLEDFAKESEQKIND
                    || seq->mo_va_kind == 0,
                    "mo_make_tuple_closeq: seq of strange kind %u",
                    (unsigned) seq->mo_va_kind);
  ...
  for (unsigned ix = 0; ix < sz; ix++)
    {
      ...
      momhash_t curh = curobjr->mo_va_hash;
...
mo_make_set_closeq (mo_sequencevalue_ty * seq)
{
  ...
  for (unsigned ix = 0; ix < sz; ix++)
    {
      mo_objref_t curobr = seq->mo_seqobj[ix];
      momhash_t curh = curobr->mo_va_hash;
      MOM_ASSERTPRINTF (curh != 0,
...
mo_set_union (mo_value_t vset1, mo_value_t vset2)
{
  ...
  const mo_setvalue_ty *set1 = (mo_setvalue_ty *) mo_dyncast_set (vset1);
  const mo_setvalue_ty *set2 = (mo_setvalue_ty *) mo_dyncast_set (vset2);
  if (set1)
    card1 = set1->mo_sva_size;
  if (set2)
    card2 = set2->mo_sva_size;
  ...
  while (i1 < card1 && i2 < card2)
    {
      mo_objref_t ob1 = set1->mo_seqobj[i1];
      mo_objref_t ob2 = set2->mo_seqobj[i2];
      MOM_ASSERTPRINTF (ob1
                        && ob1->mo_va_kind == mo_KOBJECT, "bad ob1@%p", ob1);
      MOM_ASSERTPRINTF (ob2
                        && ob2->mo_va_kind == mo_KOBJECT, "bad ob2@%p", ob2);
  ...
  if (i1 < card1)
    for (unsigned ix = i1; ix < card1; ix++)
      {
        mo_objref_t ob1 = set1->mo_seqobj[ix];
        MOM_ASSERTPRINTF (ob1
                          && ob1->mo_va_kind == mo_KOBJECT,
                          "bad ob1@%p", ob1);
        arr[nbun++] = ob1;
      }
  else if (i2 < card2)
    for (unsigned ix = i2; ix < card2; ix++)
      {
        mo_objref_t ob2 = set2->mo_seqobj[ix];
        MOM_ASSERTPRINTF (ob2
                          && ob2->mo_va_kind == mo_KOBJECT, "bad ob2@%p",
                          ob2);
        arr[nbun++] = ob2;
      }
  ...
  while (i1 < card1 && i2 < card2)
    {
      mo_objref_t ob1 = set1->mo_seqobj[i1];
      mo_objref_t ob2 = set2->mo_seqobj[i2];
      MOM_ASSERTPRINTF (ob1 && ob1->mo_va_kind == mo_KOBJECT,
                        "bad ob1@%p", ob1);
      MOM_ASSERTPRINTF (ob2 && ob2->mo_va_kind == mo_KOBJECT,
                        "bad ob2@%p", ob2);
      assert (nbdi < siz);
      if (ob1 == ob2)
        goto same;
...
mo_vectval_reserve (mo_vectvaldatapayl_ty * vect, unsigned gap)
{
  ...
  if (!vect)
    {
      ...
      vect->mo_va_kind = mo_PVECTVALDATA;
      vect->mo_va_hash = (momrand_genrand_int31 () & 0xfffffff) + 2;
      vect->mo_sva_size = sz;
      vect->mo_cpl_count = 0;
      return vect;
    }
  ...
  newvect->mo_va_kind = mo_PASSOVALDATA;
  newvect->mo_va_hash = (momrand_genrand_int31 () & 0xfffffff) + 2;
  newvect->mo_sva_size = newsz;
  memcpy (newvect->mo_vect_arr, vect->mo_vect_arr, cnt);
  newvect->mo_cpl_count = cnt;
  return newvect;
    ...
    case mo_KSET:
      {
        ...
        long ln = ftell (fmem);
        fflush (fmem);
        MOM_ASSERTPRINTF (ln >= 0 && ln <= (long) siz,
                          "bad ln=%ld siz=%zd", ln, siz);
        buf[ln] = 0;
        res = mom_gc_strdup (buf);
        fclose (fmem), fmem = NULL;
        free (buf), buf = NULL;
